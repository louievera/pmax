	...
	<v-content>
		<v-container grid-list-md text-center>
			...
			<template v-slot:items="props">
				...
				<td class="text-xs-left">{{ props.item.austrac_no }}</td>
				<td class="text-xs-left">{{ props.item.abn_no }}</td>
				<td class="text-xs-left">{{ props.item.agent_status }}</td>
				<td class="justify-center layout px-0">
					<v-icon
					small
					class="mr-2"
	...
	export default {
		data: () => ({
			...
			headers: [
				...
				{ text: 'Austrac No', value: 'austrac_no', sortable: false },
				{ text: 'ABN No', value: 'abn_no' },
		...
		methods: {
			initialize () {
				axios.get('./api/agent').then(response => this.list = response.data);				
			},
